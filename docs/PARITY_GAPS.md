...
- [x] Corpus sweep: DXSDK corpus now passes after numeric promotion (int/uint/half widening), branch arity fixes, return widening (float3->float4) on PS signatures, and deeper JSON serialization limits for large DX11 shaders (adaptive tessellation/BC6H7 decoders).***
